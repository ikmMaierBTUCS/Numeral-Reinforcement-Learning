...
class SCFunction:
        ...
        if isinstance(self,other.__class__):
            if self.root != other.root or self.mapping != other.mapping:
                ...
                for comp in range(self.dimension()):
                    r_m = set((e.root,e.mapping) for e in self.inputrange[comp])
                    other_r_m = set((e.root,e.mapping) for e in other.inputrange[comp])
                    if r_m != other_r_m:
                        return False
                return True
        else:
    ...
    def input_numberbase(self):
        ...
        base_complete = False
        number_of_variable_input_slots = len([comp for comp in range(len(self.dimension())) if len(self.inputrange[comp]) > 1])
    ...
    def vereinige(self,mergee, other_mergees = [],printb= True,trust_affinity=True):
        ...
        if np.linalg.matrix_rank(np.array(affine_basis + [[m.mapping[-1] for m in mergee_inputvector] + [1]], dtype=float), tol=1e-7) == len(build_base):
            vereinigte_funktion = SCFunction(self.root,[self.inputrange[comp] + [mergee.inputrange[comp]] for comp in range(len(self.dimension()))], self.mapping, c = [self.confirmed_inputrange[comp] + [mergee.inputrange[comp]] for comp in range(len(self.dimension()))])
        ...
        neue_dimension = len([comp for comp in range(len(self.dimension())) if not (len(self.inputrange[comp]) == 1 and not set([mergee_inputvector[comp]] + [omi[i][comp] for i in other_mergee_inputvectors]).issubset(self.inputrange[comp]))]) # l
