...
def intlinsolve(base,image):
    ...
    try:
        ...
        try:
            ...
            return [i for i in np.dot(np.linalg.pinv(np.array([b+[1] for b in base], dtype=np.float64)),image)]
        except IndexError:
            #print('unique solution')
            #return [round(i) for i in np.dot(np.linalg.pinv(np.array([b+[1] for b in base], dtype=np.float64),rcond=1e-15),image)]
            return [i for i in np.dot(np.linalg.pinv(np.array([b+[1] for b in base], dtype=np.float64)),image)]
...
def solve_lexicon_clashes(entry1, entry2):
    ...
    values2 = [(inp,round(sum([inp[i]*entry2.mapping[i] for i in range(entry2.dimension())])) + entry2.mapping[-1]) for inp in cartesian_product(entry2.number_inputs())]
    #confirmed_values2 includes all confirmed outputs
    confirmed_values2 = [round(sum([inp[i]*entry2.mapping[i] for i in range(entry2.dimension())]) + entry2.mapping[-1]) for inp in cartesian_product(entry2.number_inputs(only_confirmed = True))]
    ...
    for comp in range(entry1.dimension()):
        ...
        for entr in entry1.inputrange[comp]:
            ...
            #otherwise
                ...
                io = [(inp,round(sum([entry1.mapping[i]*inp[i] for i in range(entry1.dimension())]) + entry1.mapping[-1])) for inp in inputs_involving_entr]
...
def learn_lexicon(lexicon,supervisor,initial_lexicon=[],version='a',printb=True,normalize=True):
    ...
    supervisor_errors = []
    for voc in lexicon:
        #update statistical supervisor based on data of new confirmed word
        if supervisor.style in ['statistic','hybrid']:
            supervisor.update([voc])

        #check if word is known already
        if (type(supervisor) != list and supervisor.style in ['dummy','arithmetic']) or not any([(voc.word,voc.number) in [(outp.root,outp.mapping[-1]) for outp in entr.all_outputs()] for entr in learnerlex]):

            #check if another word of voc's value has been learned before --> supervisor error
            if supervisor.style in ['statistic','hybrid'] and voc.mapping[-1] not in supervisor.data.keys():
                for entr in learnerlex:
                    for o in entr.all_outputs:
                        if o.mapping[-1] == voc.mapping[-1]:
                            print("SUPERVISOR ERROR: " + str(voc.mapping[-1]) + " apparently means " + voc.word + " and not " + o.word)
                            supervisor_errors += [voc.mapping[-1]]
                        
            samples += 1
            #1 parse new word
            #if printb: print('What means '+str(voc.number)+'?')
            if printb: print('Supervisor: '+voc.word+' means '+str(voc.number))
            if version == 'a':
            ...
            break_length = False
            for length in reversed(range(1, len(functions_with_equal_template) + 1)):
                for sample in itertools.combinations(functions_with_equal_template, length):
                    # show samples of functions with equal template with decreasing sample size
                    copy_sample = list(sample)
                    merger = None
                    # find suitable merger
                    for entry in copy_sample:
                        if len(entry.number_inputs()) > 1:
                            merger = entry
                            copy_sample.remove(entry)
                            break
                    if merger == None:
                        merger = sample[0]
                        copy_sample = copy_sample[1:]
                    #print('Can ' + merger.root + ' merge with ' + str([i.number_inputs() for i in copy_sample+[parse]]) + '? ' + str(mergable))
                    #print('Merger: ')
                    #merger.present()
                    #print('Mergees: ')
                    #print(str([i.number_inputs() for i in copy_sample+[parse]]))
                    mergable = merger.multi_mergable(copy_sample+[parse])
                    if mergable:
                        merged = merger.multi_merge(copy_sample+[parse],printb=printb,trust_affinity=False)
                        learned = merged
                        if printb: learned[-1].present()

                        # remove merged entries
                        for entry in sample:
                            for lentry in learnerlex:
                                if entry.root == lentry.root and entry.number_inputs() == lentry.number_inputs():
                                    print('remove ' + lentry.root)
                                    learnerlex.remove(lentry)
                            

                        # reinforce learned with learnerlex
                        if len(learned)==1 and entry.actual_dimension() < learned[0].actual_dimension():
                            if printb: print('Attempting reinforcement')
                            learned[0] = learned[0].reinforce(learnerlex,supervisor,printb)
                        understood = True
            ...
            updated_entries = learned

            # reinforce learnerlex with learned
            for i in range(len(learnerlex)):
                if sum(learnerlex[i].mapping) > sum(learned[-1].mapping) and learnerlex[i].dimension() > 0: 
                    #if printb: print('Attempting to reinforce ' + learnerlex[i].root + ' with ' + learned.root)
                    new_entry = learnerlex[i].reinforce([learned[-1]],supervisor,printb,aux_lex=learnerlex)
                else:
            ...
            if supervisor.style == 'arithmetic':
                for entry in learnerlex:
                    learned[-1] = solve_lexicon_clashes(learned[-1],entry)
                for entry in learnerlex:
                    entry = solve_lexicon_clashes(entry,learned[-1])
    ...
    if printb and supervisor.style == 'statistic': print(str(len(supervisor_errors)) + ' supervisor errors occurred. They affected the numbers ' + ', '.join(supervisor_errors) + '.')
...
def learn_language(language,version='a',set_limit=10**9,printb=True,normalize=True):
    print('Learning '+language)
    supervisor = create_lexicon(language,set_limit=set_limit)
    learnerlex = []
    samples = 0
    for voc in supervisor:
        if not any([voc.word in [outp.root for outp in entr.all_outputs()] for entr in learnerlex]):
            ...
            if printb: print('Supervisor: '+voc.word)#+' means '+str(voc.number))
            if version == 'a':
                parse = advanced_parse(voc.number, voc.word, learnerlex, False, printb)
            elif version == 'p':
                parse = proto_parse(voc.number, voc.word, learnerlex, False, printb)
            # Understood?
            understood = False
            for entry in learnerlex:
                if entry.root == parse.root:
                    ...
                    #try:
                    if True:
                        learned = entry.merge(parse,printb=printb)
                        learnerlex.remove(entry)
                        if printb: learned.present()
                        #print(str(entry.actual_dimension()) + ' < ' + str(learned.actual_dimension()) + ' ?')
                        if entry.actual_dimension() < learned.actual_dimension():
                            if printb: print('Attempting reinforcement')
                            learned = learned.reinforce(learnerlex,supervisor,printb)
                        understood = True
                        break
                    #except:
                    if False:
                        if printb: print('Not related to '+entry.root)
                        pass
            if not understood:
                #2b No, just remembering
                learned = parse
    ...
    #reorganize all inputranges so that they only contain scfunctions, no vocabulary
    if normalize: learnerlex = normalize_scf_lexicon(learnerlex,printb)
    if printb: print('Learned '+str(len(supervisor))+' words in '+language+' and structured them in '+str(len(learnerlex))+' functions.')
    if printb: print('It took '+str(samples)+' samples to learn those.')
    if printb: print('Those are:')
    for entry in learnerlex:
        if printb: entry.present()
...
'''
def normalize_scf_lexicon(learnerlex):
    new_learnerlex = []
                ...
                if isinstance(entr,SCFunction):
                    ...
                    for scf in learnerlex:
                        if isinstance(scf,SCFunction) and parse.root == scf.root and entr.root in [op.root for op in scf.all_outputs()]:
                            print('In domain of ' + lexentr.root + ', ' + entr.root + ' can be covered by ' + scf.root)
...
for language in ['he','Persian', 'Tunica', 'Gilbertese','Haida','Susu','Dogrib','Nume','Yao','Choapan-Zapotec']: #sprachen:
    total_lexicon = create_lexicon(language, set_limit = learning_range)
    if True: #len(total_lexicon) > 998 and not language in list(data.keys()):
        knowledge = {v.mapping[-1]:v.root for v in total_lexicon}
        supervisor = Oracle('omniscient', knowledge = list(knowledge.values()))
        data[language] = []
        ...
        for i in range(1):
            ...
            recognized_numerals = []
            learner_lex = []
            for training_size in range(learning_range):
                if training_size % step_width == 0 or training_size == learning_range - 1:
                    ...
                    # learn
                    learner_lex = learn_lexicon(lexicon,supervisor,initial_lexicon=learner_lex,printb=True,normalize=True)
                    
                    # evaluate
                    unlearned_numbers = []
                    errors = 0
                    lex = sorted(learner_lex,key=lambda x:x.root.count('_'))
                    for n in range(1,learning_range):
